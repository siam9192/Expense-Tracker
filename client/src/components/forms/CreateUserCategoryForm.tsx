  ...
  return (
    <form action="" onSubmit={submit} className="p-2 md:p-5 space-y-4">
      <fieldset className="fieldset ">
        ...
        <input  type="text" {...register("name")} className="input input-lg w-full" placeholder="Type here" />
         {errors.name && <p className="text-sm text-error mt-1">{errors.name.message}</p>}
      </fieldset>

      <fieldset className="fieldset ">
        <legend className="fieldset-legend">Type</legend>
        <select {...register("type")} defaultValue={CategoryType.INCOME} className="select select-lg w-full">
          {categoryTypes.map((_) => (
      ...
      <button type="submit" onClick={submit} disabled={isLoading} className="btn btn-primary w-full">Submit</button>
