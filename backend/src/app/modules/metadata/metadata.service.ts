...
async getCurrentUserGoalsSummary(authUser: AuthUser) {
  ...
  const avg_progress =
    goals.reduce((sum, g) => sum + g.complete_percentage, 0) /
    goals.length;

  const total_completed_goals =
...
async getCurrentUserExpenseStats(
  ...
  for (const tx of expenses) {
    ...
    else if (sequence === "month") key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
...
async getCurrentUserExpenseCategoryBreakdown(
  ...
  const breakdown = categories.map((cat) => {
    ...
    const percentage = totalExpenses > 0 ? Number(((total / totalExpenses) * 100).toFixed(2)) : 0;
